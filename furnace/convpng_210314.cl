...
begin
	...
	int ii, jj, inlen, outlen, intime, outtime, iverb
	string infile, iroot, outfile, modfile
...
# They do have an effect on ograph plotted to xgterm.
	  ...
	  if ( iverb > 4 ) { print ("convpng: input ",infile, " ", intime) }

	  # Check if output file can be created or clobbered
	  if ( !access(outfile) || clobber ) {

	     outtime = 0
	    # if output file exists, check modification time
	     if ( access(outfile) ) {
	     	print ("!stat -c %Y "//outfile) | cl | scan (outtime)
	     	if ( iverb > 4 ) { print ("convpng: old ",outfile, " ", outtime) }
	     }
	     
	     #if ( intime > outtime ) {print ("mc is newer than png")}
	     #if ( intime < outtime ) {print ("mc is older than png")}


             # check for newer flag, and skip if output file is newer
	     if ( !newer || ( intime > outtime ) ) {
	     	 if ( black ) {
		     if ( iverb > 2 ) {	print ("convpng: Editting metacode file to ",modfile) }
		     # edit input metacode file
		     print ("!/home/jhill/scopes/furnace/blackfont.bash ",infile," ",modfile) | cl
		 }
		 else {
		     modfile = infile
		 ...
		 if ( access(outfile) ) {
	     	     print ("!stat -c %Y "//outfile) | cl | scan (outtime)
	     	     if ( iverb > 3 ) { print ("convpng: new ", outfile, " ", outtime) }
	         }
