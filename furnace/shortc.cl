...
# Added autoscale to fexport, 22MAY00
# Cam E and Cam B commented out for LBT2 anneal.  30MAY00
# All cameras to start LOTIS casting. 29MAR02
# Cam B commented out for GMT1 casting. 20JUL05
# Cam B restored for LSST, Cam A using flashes AD. 31MAR08
...
begin
  ...
  if ( verb > 2 ) {
	...
	print ("  version of 31MAR08,   vindex = ", vindex)
  } # end if

  if ( strlen(vlogfile) > 0 ) {
	print ("Starting new copy of shortc.cl", >> vlogfile )
	print ("  version of 31MAR08,   vindex = ", vindex, >> vlogfile )
  } # end if
  ...
  while ( 0 <  1) {
    ...
    ovenw
    takepic ( vroot, camera="b", flash="c", exposure=6,
	index=vindex, pname=vpath, xname=vtemp, xhost=vhost, auto=vauto, 
	logfile=lastfile, clobber=yes, verb=verb )
