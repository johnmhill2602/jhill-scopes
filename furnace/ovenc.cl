...
# updated whole firing parameters for SAO prefire, RDL 23Aug10
# uncommented rotation speed plot for GMT2, JMH 13Jan12
# updated whole firing parameters for GMT3 casting. RDL 19Aug13
# uncomment rotation speed plot and check rspeed existence, JMH 23Aug13
# updated whole firing parameters for GMT4 casting. JMH 14Sep15
# uncomment rotation speed plot.  JMH 17Sep15
# updated whole firing parameters for GMT5 casting. RDL 30Oct17
# updated whole firing for 6.5m band seat fire. RDL 01Oct18
# updated whole firing for TBDM 6.5m prefire. RDL 18Mar19
# updated whole firing for VGT 6.5m casting.  RDL 21April19
# updated whole firing for VGT 6.5m re-casting.  RDL 15Aug2019 
# updated whole firing for GMT6 prefire.  JMH 21JAN2021
# updated whole firing for GMT6 casting.  RDL 24Feb2021
# added high temp plot for GMT6 casting.  JMH 06MAR2021 (then commented out)
# commented out the first odisp  JMH 12APR2021
# restored the first odisp JMH 13APR2021
...
if ( strlen(ologfile) > 0 ) {
	...
	print ("  version of 13APR2021", >> ologfile )
} # end if


i = 0 # counter to not make whole casting plot too often

    
# Continuous loop of plotting starts here
    while (0<1) {
...
# odisp the base ttmp map
	odisp (info="ttmp",asp="b",frame=3, debug=yes) 
...
# ograph the last 8 hours
	ovenw;ograph (dateleft=INDEF,dateright=INDEF,
		timelori="now",timerori="now",timeloff=-800,timeroff=10,
		yhiori="maxdata",yloori="mindata",yhioff=2,ylooff=-2,
		info1="etmp",zone1="z0",factor1=1,
		info2="ztmp",zone2="z0",factor2=1,
		info3="ztmp",zone3="z1",factor3=1,
...
# ograph the last 3 days
	ovenw;ograph (dateleft=INDEF,dateright=INDEF,
		timelori="now",timerori="now",timeloff=-7200,timeroff=80,
		yhiori="maxdata",yloori="mindata",yhioff=2,ylooff=-2,
		info1="etmp",zone1="z0",factor1=1,
		info2="ztmp",zone2="z0",factor2=1,
		info3="ztmp",zone3="z1",factor3=1,
...
# only need to do this every half hour or so
# ograph the whole Casting
  	if ( i < 1 ) {
  	   ovenw;ograph (dateleft=210301,dateright=INDEF,
		timelori="",timerori="now",timeloff=0100,timeroff=120,
	   ...
	   ograph (dateleft=210301,dateright=INDEF,
		timelori="",timerori="now",timeloff=0100,timeroff=120,
		yhiori="zero",yloori="zero",yhioff=1200,ylooff=0,
		info1="etmp",zone1="z3",factor1=1,
		info2="ztmp",zone2="z4",factor2=1,
	...
	# increment the counter for skipping the whole casting plot
	i = i + 1
	if ( i > 5 ) { i = 0 }


# ograph the high temperature period
#	ovenw;ograph (dateleft=210306,dateright=210306,
#		timelori="",timerori="",timeloff=0940,timeroff=2140,
#		yhiori="zero",yloori="zero",yhioff=1200,ylooff=1000,
#		info1="etmp",zone1="z0",factor1=1,
#		info2="ztmp",zone2="z0",factor2=1,
...
#		timeunit="hours",
#		append=no,subsample=1,device="stdvdm")
#	delete ( "ohigh.mc", verify=no )
#	rename ( "uparm$vdm", "ohigh.mc" )
#	gkimosaic ( "ohigh.mc", nx=1, ny=1, interactive- )
...
# graph the oven rotation speed (captured by mkpngc in 2021)
# # # spinplot requires the scopes and furnace external IRAF packages to be loaded.
#	if (!defpac ("scopes"))
#    		scopes
#	if (!defpac ("furnace"))
#    		furnace
# # # spinplot also requires the ovenr daemons to be making the rspeed files.
#	if ( access("/home/pilot/rspeed.oven0v0") && access("/home/pilot/rspeed.oven0v2") ) {
#		ovenw
#		spinplot (rfile="/home/pilot/rspeed.oven0v0", izero="",
#		    istart=-2500,istop=INDEF,iincr=2500,rmin=4600,rmax=5100,
#		    append=no,device="stdvdm")
