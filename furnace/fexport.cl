...
# Version 29AUG09 -- dark subraction
...
bool	darksub=no		{prompt="subtract dark images?"}
...
string	version="29 Aug 2009" {prompt="Version date of this routine"}
struct	*list1

# Warning: assumes that FITS extension is ".fts"
# Warning: assumes that dark image name is "dark" (replacing 4 chars)

begin

	# define variables
	string rname, sname, jname, kname, lname, gname, temp1, ename
	string rdark, jdark, rpad
	...
	while ( fscan(list1, gname) != EOF) {	
		...
		# Check scaling before dark subtraction
		if ( autoscale ) {
		...
		print ( substr(rname,stridx(".",rname)-5,stridx(".",rname)-2) ) | scan ( x1 )
		x1 = x1 - 1
		rpad = ""
		if (x1 < 1000) rpad = "0"
		if (x1 < 100) rpad = "00"
		...
		# make the name of the dark image (assumes format of videoc)
		rdark = substr(rname,1,stridx(".",rname)-10) // "dark" 
		rdark = rdark // rpad // str(x1) 
		rdark = rdark // substr(rname,stridx(".",rname)-1,strlen(rname))

		# make IRAF name for dark image (assume extension was .fts)
		jdark = substr(rdark, 1, strlen(rdark)-4) // ".imh"
		while (stridx("/", jdark) > 0) {
		# strip leading directory paths
		    jdark = substr(jdark,stridx("/",jdark)+1,strlen(jdark))
		} #end while
		while (stridx("$", jdark) > 0) {
		# strip leading indirect path
		    jdark = substr(jdark,stridx("$",jdark)+1,strlen(jdark))
		...
		# print ( "Dark image: ", rdark )

		# read the FITS image with task rfits
		rfits (rdark, file_list="*", iraf_file=jdark, make_image=yes,
		long_header=no, short_header=no, datatype="",
		scale=no, oldirafname=no, offset=0)

		imarith ( jname, "-", jdark, jname )
