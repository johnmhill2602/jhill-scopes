...
# added processing for onowa plot. JMH 19SEP19
#     should it pick newest onow plot?
...
if ( strlen(ologfile) > 0 ) {
	...
	print ("  version of 19SEP19", >> ologfile )
} # end if
	...
	ovenw (offset=35)
	if ( access("onowc.mc") ) {
	    gkimosaic ( "onowc.mc", nx=1, ny=1, interact-, device="psidump" )
	    gflush
	    psi2gif ( "home$public_html/frame6.gif", clobber+, clean+, verb=2 )
	}
	!fetch.cgi
	temp
	...
	ovenw (offset=35)
	if ( access("onowa.mc") ) {
	    gkimosaic ( "onowa.mc", nx=1, ny=1, interact-, device="psidump" )
	    gflush
	    psi2gif ( "home$public_html/frame6.gif", clobber+, clean+, verb=2 )
	}
	!fetch.cgi
