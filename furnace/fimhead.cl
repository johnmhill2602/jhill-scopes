procedure fimhead (fname)
# cl script to convert a FITS image to IRAF format and imhead it.
# Created 25JAN94 by J. M. Hill, Steward Observatory
# Version 07JAN97 -- Temporary files in tmp$.
# Version 14SEP98 -- fits file "*"
# Version 21JUL05 -- verbosity check

string  fname			{prompt="list of FITS images to read"}
bool	long=yes		{prompt="long header"}
string  iname=""		{prompt="temporary IRAF image name"}
bool	isave=no		{prompt="save IRAF image"}
int	verb=4			{prompt="verbosity"}
string	version="21 July 2005" {prompt="Version date of this routine"}
struct	*list1

begin
	# define variables
	string rname, jname, gname, temp1
	...
	while ( fscan(list1, gname) != EOF) {	
	    ...
	    if ( access(rname) ) {
	        ...
	        # imhead the image using the preset imhead parameters
	        imhead (jname, long=long)
