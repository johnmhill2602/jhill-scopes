...
# T_GKIMERGE -- Hacked from T_GKIMOSAIC in plot$
# J. M. Hill, Steward Observatory  10AUG93
# --  Plot multiple metacode frames on a single output page.
# Input is read from either STDIN or a metacode file; output can be
# sent directly to a named device or a metacode file.
# The plots are merged 1:1 without any scaling, and they all go on one page.

procedure t_gkimerge ()
...
long	fpos, length_mc
bool	clgetb(), streq()
int	open(), clpopni(), clgfil(), btoi(), fstati(), gm_interact()
int	clplen()
long	gm_rwframe()
...
begin
	...
	# Get remaining cl parameters
        nx = 1 # JMH nx = max (1, clgeti ("nx"))
        ny = 1 # JMH ny = max (1, clgeti ("ny"))
	nplots_page = nx * ny
	fill = clgetb ("fill")
