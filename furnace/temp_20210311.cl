...
# declarations
	...
	string wday, month, junk1, junk2, year
	int iday, minutes, index1, index2, index3, index4
	real hours
	string yy, mm, dd
...
# get the date
	#date | scan(wday, month, iday, hours, junk1, year)         #SunOS
	date | scan(wday, iday, month, year)  #Fedora 
	time | scan(wday, hours)  #Fedora to get 0-23

	minutes = (hours * 60) - 1 # hours was sexagesimal
...
# process the first zone if zone number was >=0
	...
	if ( iday < 10 ) {
		datafile1 = '/home/pilot/ztmp'//yy//mm//'0'//str(iday)//'.fits[1:7,'//str(minutes)//']'
		# print (datafile1)
		wtextimage (datafile1, outfile1, header=no, pixels=yes, format="10.1f")
	}
	else {
		datafile1 = '/home/pilot/ztmp'//yy//mm//str(iday)//'.fits[1:7,'//str(minutes)//']'
		#print (datafile1)
		wtextimage (datafile1, outfile1, header=no, pixels=yes, format="10.1f")
	}
    } # end if index1
...
# process the second zone if zone number was >= 0
	...
	if ( iday < 10 ) {
		datafile2 = '/home/pilot/ztmp'//yy//mm//'0'//str(iday)//'.fits['//str(index2)//','//str(minutes)//']'
		#print (datafile2)
		wtextimage (datafile2, outfile2, header=no, pixels=yes, format="10.3f")
	}
	else {
		datafile2 = '/home/pilot/ztmp'//yy//mm//str(iday)//'.fits['//str(index2)//','//str(minutes)//']'
		#print (datafile2, " ", outfile2)
		# WHY IS THE FOLLOWING COMMAND NOT MAKING OUTPUT? 
		wtextimage (datafile2, outfile2, header=no, pixels=yes, format="10.3f")
	}
    } # end if index2
...
# process the third zone if zone number was >=0
	...
	if ( iday < 10 ) {
		datafile3 = '/home/pilot/zsct'//yy//mm//'0'//str(iday)//'.fits[1:7,'//str(minutes)//']'
		# print (datafile3)
		wtextimage (datafile3, outfile3, header=no, pixels=yes, format="10.1f")
	}
	else {
		datafile3 = '/home/pilot/zsct'//yy//mm//str(iday)//'.fits[1:7,'//str(minutes)//']'
		# print (datafile3)
		wtextimage (datafile3, outfile3, header=no, pixels=yes, format="10.1f")
	}
    } # end if index3
...
# process the fourth zone if zone number was >=0
	...
	if ( iday < 10 ) {
		datafile4 = '/home/pilot/hpwr'//yy//mm//'0'//str(iday)//'.fits[1:270,'//str(minutes)//']'
		# print (datafile4)
		imstat (datafile4, fields="mean,stddev,min,max,npix", format=no, > outfile4)
	}
	else {
		datafile4 = '/home/pilot/hpwr'//yy//mm//str(iday)//'.fits[1:270,'//str(minutes)//']'
		# print (datafile4)
		imstat (datafile4, fields="mean,stddev,min,max,npix", format=no, > outfile4)
	}
    } # end if index4
